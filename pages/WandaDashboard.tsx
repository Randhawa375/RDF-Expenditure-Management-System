...
const WandaDashboard: React.FC = () => {
    ...
    const fetchRecords = async () => {
        setIsLoading(true);
        try {
            const data = await db.getWandaRecords();
            setRecords(data);
        } catch (error) {
    ...
    const stats = useMemo(() => {
        return records.reduce((acc, r) => {
            acc.totalBags += r.bags;
            acc.totalDue += (r.bags * r.price_per_bag);
            acc.totalPaid += r.payment_given;
            return acc;
        }, { totalBags: 0, totalDue: 0, totalPaid: 0 });
    }, [records]);

    const remainingBalance = stats.totalDue - stats.totalPaid;

    const handleSave = async (record: WandaRecord) => {
        try {
            await db.saveWandaRecord(record);
            fetchRecords();
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mb-1">Total Bags</p>
                    <h2 className="text-2xl font-black text-slate-900">{stats.totalBags.toLocaleString()}</h2>
                    <p className="text-slate-400 font-urdu mt-1 italic text-xs">کل بوریاں</p>
                </div>
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-100">
                                ...
                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Bags</th>
                                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Price/Bag</th>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {records.map((r) => (
                                <tr key={r.id} className="hover:bg-slate-50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 text-sm font-bold text-slate-700">{r.bags}</td>
                                    <td className="px-6 py-4 text-sm font-bold text-slate-700">{r.price_per_bag.toLocaleString()}</td>
                                    <td className="px-6 py-4 text-sm font-black text-slate-900">{(r.bags * r.price_per_bag).toLocaleString()}</td>
