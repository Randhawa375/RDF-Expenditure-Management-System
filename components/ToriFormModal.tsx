...
interface ToriFormModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (record: ToriRecord) => void;
    editRecord: ToriRecord | null;
    mode?: 'FULL' | 'PAYMENT_ONLY';
}

const ToriFormModal: React.FC<ToriFormModalProps> = ({ isOpen, onClose, onSave, editRecord, mode = 'FULL' }) => {
    const [date, setDate] = useState(() => new Date().toISOString().split('T')[0]);
    const [mun, setMun] = useState('');
    const [pricePerMun, setPricePerMun] = useState('');
    ...
    useEffect(() => {
        if (editRecord) {
            ...
            setMun(editRecord.mun.toString());
            setPricePerMun(editRecord.price_per_mun.toString());
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const publicUrl = await db.uploadFile('slips', `tori/${fileName}`, file);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onSave({
            ...
            date,
            mun: Number(mun),
            price_per_mun: Number(pricePerMun),
            ...
            <div className="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-300">
                <div className="bg-indigo-900 px-6 py-5 flex justify-between items-center">
                    <div>
                        <h2 className="text-xl font-black text-white tracking-tight">
                            {editRecord ? 'Edit Tori Record' : isPaymentOnly ? 'Add Tori Payment' : 'Add Tori Mun'}
                        </h2>
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-0.5 font-urdu">
                            {editRecord ? 'ریکارڈ تبدیل کریں' : isPaymentOnly ? 'رقم کی ادائیگی' : 'من شامل کریں'}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Weight (Mun)</label>
                            <input
                                type="number"
                                step="0.01"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Price Per Mun</label>
                            <input
                                type="number"
                                required={!isPaymentOnly}
                                disabled={isPaymentOnly}
                                value={pricePerMun}
                                onChange={(e) => setPricePerMun(e.target.value)}
                                className={`w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-700 outline-none ${isPaymentOnly ? 'opacity-50' : ''}`}
                                placeholder="0"
                            />
                        </div>
                    ...
                    <button
                        type="submit"
                        className="w-full bg-indigo-900 hover:bg-slate-800 text-white font-black py-4 rounded-xl shadow-xl transition-all active:scale-95 text-sm uppercase tracking-widest"
                    >
